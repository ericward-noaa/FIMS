...
`fims_model()` starts from a `FIMSFrame` object. Components are then added with native R pipes. Observation components are specified by fleet, so fleet-specific choices such as selectivity and likelihood distributions live next to the data they describe.

```{r model-components, max.height='100px', attr.output='.numberLines'}
...
After enough components are added, `model$parameters` contains the default values for parameters that relate to fleet(s), recruitment, growth, and maturity modules. For example,
