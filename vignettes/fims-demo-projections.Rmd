...
The code below uses the built-in data within FIMS and sets up a simple catch-at-age model that will work with the data. This data and model structure will be used to create an extended data set in the next section. More details about the chunk below can be found in the [introductory vignette](fims-demo.html).
...
The same component stack from a model without projections can be used with the projection data. After the parameters are created they can be manipulated to alter defaults for things like selectivity, fishing mortality, etc. So, much of what is done below for projections was also done in the [introductory vignette](fims-demo.html).
...
```{r projection-model}
# Build the same component stack with the projection data, then alter
# parameters to make the model behave a little better.
parameters_projection <- fims_model(data_4_projections) |>
  fims_growth() |>
  fims_recruitment() |>
  fims_maturity() |>
  fims_observations(fleet = "fleet1") |>
