...
The R wrapper functions use a component-building workflow before `initialize_fims()` constructs the TMB-ready objects:

1. **`fims_model()` plus `fims_*()` component functions** declare the pieces that should be used.
2. **`model$parameters`** stores the parameter tibble that `initialize_module()` reads.

### 4.1 Update the configuration and default-parameter path

**Relevant files**

- [R/fims_model.R](../R/fims_model.R)
- [R/create_default_parameters.R](../R/create_default_parameters.R)

If you add a new module type that users should be able to request through the builder, update:

- the component builder and internal parameter-plan logic in `fims_model.R`, and/or
...
- a valid `module_type` value in the parameter tibble,
...
If your new module breaks any part of that pipeline, update the corresponding component-builder, parameter, initialization, and registration code together.
...
- [ ] `R/fims_model.R` if the new module type should appear in the component builder
...
4. **R component-builder, parameter, and initialization workflow** in `R/fims_model.R`, `R/create_default_parameters.R`, and `R/initialize_modules.R`
